...
def get_urls(path, view_choice, sample):
    ...
    with open(path, "w") as urls_text_file:
        ...
        sub_pages = 19  # 16 because for each type there are 16 pages of links in the site
        if sample:
            sub_pages = 2
        #for name in texts_types:
        for page_number in range(1, sub_pages+1):
            page_link = "http://www.achemenet.com/fr/tree/?/sources-textuelles/" \
                        "textes-par-publication/Strassmaier_Cambyse/%s/24/0" % (page_number)
            ...
            for line in lines:
                line = str(line)
                if line.find("Strassmaier_Cambyse/167") >= 0 or line.find("Strassmaier_Cambyse/168") >= 0:
                    url_list.append("http://www.achemenet.com/" + line.split('"')[3])
                    urls_text_file.write("http://www.achemenet.com/" + line.split('"')[3] + "\n")
                    # There are 384 texts
                    print_progress(percentage, 441, view_choice, "http://www.achemenet.com/" + line.split('"')[3])
                    percentage += 1
    forbidden_urls = [
...
def clean_text(text, url):
    ...
    try:
        ...
        text = '\n'.join([line for line in text.split('\n')
                          if not (line.strip().isdecimal() or line.strip() == '' or
                                  'Collations' in line.strip() or 'érasée' in line.strip())])
    ...
    with open(path, "w", encoding="utf-8") as texts_file:
        ...
        for url in url_list:
            ...
            try:
                ...
                soup = str(soup.find(id="item-note-fields-div"))
                #soup = str(soup)
                soup = soup.replace('<i>', '')
                soup = soup.replace('</i>', '')
...
if __name__ == '__main__':
    ...
    texts_url = main_path + "/Cambyse_texts.txt"
    ...
    get_texts(main_path + "/Cambyse_texts.txt", user_view_choice, urls)
