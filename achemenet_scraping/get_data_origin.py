...
def get_urls(path, view_choice, sample):
    ...
    with open(path, "w") as urls_text_file:
        ...
        sub_pages = 8  # 8 because for each type there are 8 pages of links in the site
        if sample:
            sub_pages = 2
        for name in texts_types:
            for page_number in range(1, sub_pages):
                page_link = "http://www.achemenet.com/fr/tree/?/sources-textuelles/" \
                            "textes-par-langues-et-ecritures/babylonien/%s/%s/96/0" % (name, page_number)
                page_code = urllib.urlopen(page_link)
                for line in page_code.readlines():
                    if line.find("Strassmaier") >= 0 or line.find("YOS") >= 0 or line.find("Wunsch") >= 0 \
                            or line.find("Murašu") >= 0 or line.find("Jursa") >= 0:
                        url_list.append("http://www.achemenet.com/" + line.split('"')[3])
                        urls_text_file.write("http://www.achemenet.com/" + line.split('"')[3] + "\n")
                        # There are 1834 texts
                        print_progress(percentage, 1834, view_choice, "http://www.achemenet.com/" + line.split('"')[3])
                        percentage += 1
    forbidden_urls = [
...
def get_texts(path, view_choice, url_list):
    ...
    with open(path, "w") as texts_file:
        percentage = 1.0
        for url in url_list:
...
if __name__ == '__main__':
    ...
    urls = get_urls(url_path, user_view_choice, args.sample)
    print("\nFinished download the urls")

    print("\nStarting texts download to: " + texts_url)
    get_texts(main_path + "/texts.txt", user_view_choice, urls)
