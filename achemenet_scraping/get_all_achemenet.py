...
def get_texts(path, view_choice, url_list):
    ...
    with open(path, "w", encoding="utf-8") as texts_file:
        ...
        try:
            url_head = "http://www.achemenet.com"
            url = "http://www.achemenet.com/fr/tree/?/sources-textuelles/" \
            "textes-par-publication/"
            ...
            for div in mydivs:
                try:
                    ...
                    if "textes-par-publication/" in link:
                        if "http" not in link:
                            link = url_head + link
                        page_code1 = urlopen(link)
                        soup1 = BeautifulSoup(page_code1, 'html.parser')
                        # Put spaces before and after so italic will always be a word alone
                        soup1 = soup1.find(id="tree-structure-div")
                        mydivs1 = soup1.findAll('a')
                except:
                    continue
            soup = soup.find_all(recursive=True, name='parent')
            soup = soup.find(recursive=True, name='class')
            #soup = str(soup)
            soup = soup.replace('<i>', '')
            soup = soup.replace('</i>', '')
